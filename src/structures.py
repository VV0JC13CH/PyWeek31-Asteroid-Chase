...
def make_polyrock_texture(vertices,name):
    """
    ...
    draw.polygon([(v[0]-minx,res_y-(v[1]-miny)) for v in vertices], fill=(100,100,100))
    draw.line([(v[0]-minx,res_y-(v[1]-miny)) for v in vertices]+[(vertices[0][0]-minx,res_y-(vertices[0][1]-miny))], fill=(200,200,200), width=5)
    #name = "{}:{}:{}".format("rock_texture", diameter, color)  # name must be unique for caching
...
class Structure(arcade.Sprite):
    ...
    def __init__(self, parent, space, verticies, name, type='rock'):
        ...
        (self.texture, position) = make_polyrock_texture(verticies, name)
